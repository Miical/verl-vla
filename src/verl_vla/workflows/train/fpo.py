...
from verl_vla.trainer.fpo import FPORayTrainer
...
def run_fpo(config):
    ...
    return ray.get(_run_fpo_remote.options(**remote_options).remote(config))


@ray.remote
def _run_fpo_remote(config):
